...
namespace pandore {
    ...
    *
    * The class <code>GEdge</code> defines a undirected edge between two nodes.
    * An edge is characterized by a weight and the index of the connected node.
    */
   class GEdge{
   private:
   ...
   public :
      ...
      /**
       * Adds the specified node with the specified weight and specified identifier.
       * in front of the list of connected nodes
       * @param n	the specified node.
       * @param w	the weight of the edge.
       * @return the new of list of connected nodes.
       ...
       * @return	the new index of the object.
       */
      Ulong Item( Ulong i ) { return item=i; }
   };
   ...
   public :
	 ...
	 return Add(node,item,Point2d(y,x));
       ...
       * @param add	if true add the weight to the current weight, set the new value otherwise.
       * @return	SUCCESS or FAILURE.
       */
      ...
      /**
       * Unlinks the two specified nodes by a specified edge.
       * s1 will no longer be a neighbour of s2 (and vice versa).
       * @param s1	the first node.
       * @param s2	the second node.
       * @param i       the identifier of the edge.
    ...
    * node is active in the representation of the graph or not.
    * A node can be any of the Pandore object. It indexes an 
    * item in a separate array of objects. (The secret is to use an
    * item as a pointer to a specific objet in an array [no type]).
    * <br>For the use of Graph3d see @ref graph_page.
    */
   ...
   public :
	 ...
	 return Add(node,item,Point3d(z,y,x));
       ...
       * @param s2		the second node.
       * @param weight	the weight for the link.
       * @param add	if true add the weight to the current weight, set the value otherwise.
       * @return	SUCCESS or FAILURE.
       */
      Errc Link( Long s1, Long s2, Double weight=1.0F, bool add=false );
