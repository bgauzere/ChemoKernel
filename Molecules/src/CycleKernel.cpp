...
CycleKernel::CycleKernel(KernelType kernel_type, double sigma){
  switch(kernel_type)
    {
...
void CycleKernel::selectTreelets(vector<string> treelets[SIZE_SPECTRUM])
{
  k->selectTreelets(treelets);
}
void CycleKernel::weightTreelets(map<string, double,bool (*)(string, string)> treelets[SIZE_SPECTRUM])
{
  k->weightTreelets(treelets);
}

double CycleKernel::operator() (Collection* c1, Collection* c2)
{
  double scalar = 0.0;
  treelet_spectrum ** spec_1 = (treelet_spectrum **) c1->GETVALUE("cycle_spectrum",Long);
  treelet_spectrum ** spec_2 = (treelet_spectrum **) c2->GETVALUE("cycle_spectrum",Long);
