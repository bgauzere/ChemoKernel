/*
 * @file moleculesClassificationKpp.cpp
 *
...
{	
  ...
  switch(options.kernel)
    ...
    case 3:
      cout << "Kernel : Edit Distance Kernel" << endl;
  ...
  CImg<double> gram = trainset.getGramMatrix ();
  double accuracy = 0.0;
  ...
  for (unsigned int i=0; i<N; ++i)
      ...
      for(unsigned int j=0; j<N; ++j)
	{
      ...
      cout << trainset.getMoleculeFilename(i) << " -> " << 
	trainset.getMoleculeFilename(j_max) << endl;
      int x = trainset.getParameter(j_max);      
      if(!options.quietMode)
