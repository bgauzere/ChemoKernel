...
void leaveOneOut (MoleculesDataset & dataset ,double lambda,bool quietMode,
		  double sigma,   double alpha, GraphEditDistance* edit)
{	
  double err_moy = 0;
  double std_dev = 0.0;
  double err_quad = 0.0;
  ...
  LaplacianKernel* kgraph_original = new LaplacianKernel (edit, dataset, sigma, 1.0, lambda);
  //Recopie la matrice K de kgraph_original
  dataset.computeGramMatrix(kgraph_original, false);
  ...
  for (int i=0; i<N; ++i)
    {
      ...
      LaplacianKernel* kgraph_current = new LaplacianKernel (kgraph_original);
      kgraph_current->fastAdd(col,0.00000001); //K est calculé
      // dataset.simpleAdd(col,NULL,bp_exp); //MoleculeGraph non utilisé dans KernelRidge
      // dataset.computeGramMatrix(kgraph_current);//On copie K ds gram
      KRegression* kr = new KernelRidge (kgraph_current, &dataset, alpha);
...
void showUsage ()
{
  ...
  cerr << "options:" << endl;
  cerr << "-l lambda :  Set the lambda regularization parameter of the Graph Laplacian kernel" << endl;
...
{	
  ...
  leaveOneOut(dataset, options.lambda,options.quietMode, options.sigma, options.alpha,edit);
  dataset.destroy();
